...
export class EnvironmentOrFileVariableHoverProvider implements HoverProvider {
    ...
    public async provideHover(document: TextDocument, position: Position, token: CancellationToken): Promise<Hover | undefined> {
        ...
        if (await FileVariableProvider.Instance.has(selectedVariableName, document)) {
            ...
            if (!warning && !error) {
                const contents: MarkdownString[] = [new MarkdownString(JSON.stringify(value, null, 2)), new MarkdownString(`*File Variable* \`${name}\``)];
        ...
        if (await EnvironmentVariableProvider.Instance.has(selectedVariableName)) {
            ...
            if (!warning && !error) {
                const contents: MarkdownString[] = [new MarkdownString(JSON.stringify(value, null, 2)), new MarkdownString(`*Environment Variable* \`${name}\``)];
                return new Hover(contents, wordRange);
            }
        }
